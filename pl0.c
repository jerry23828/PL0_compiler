			...
			else
			{
				...
				else
				{
					if (sym == ifsym)   /* 准备按照if语句处理 */
					{
						...
						cx1 = cx;   /* 保存当前指令地址 ,该指令是if中的条件生成的指令，判断结果存入栈顶*/
						gendo(jpc, 0, 0);   /* 生成条件跳转指令，如果，IF为真，不跳转，为假则跳转，跳转地址未知，暂时写0 */
						statementdo(fsys, ptx, lev);    /* 处理then后的语句 */
						cx2=cx;//此时执行完then语句生成的指令的最后一条 
						gendo(jmp,0,0);//生成直接跳转指令，如果执行了then后面的语句处理，则直接跳过else语句处理 
						code[cx1].a = cx;   /* 经statement处理后，cx为then后语句执行完的位置，它正是前面未定的跳转地址 ，cx++到了else语句位置，如果if条件为假，则跳到当前else位置*/
						//处理else语句 
						if(sym==elsesym)
						{
							getsymdo;
							statementdo(fsys,ptx,lev);
							code[cx2].a=cx;//cx++到if-else语句末尾,也是jmp指令要跳转到的地方 
						} 
...
void interpret()
{
	...
	do {
		...
		case sta:                        // 栈顶的值存到数组下标为a的内存
			...
			break;
		case lda:
			s[t - 1] = s[base(i.l, s, b) + s[t - 1]];       //对数组的读取 第二个S[T]是放入偏移地址的				
			break;
		case arrcheck:
			s[t] = i.a;
